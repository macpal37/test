...
class MySQLDatabaseHandler(object):
    ...
    def validate_connection(self):
        ...
        engine = db.create_engine(f"mysql+pymysql://{self.MYSQL_USER}:{self.MYSQL_USER_PASSWORD}@{self.MYSQL_HOST}:{self.MYSQL_PORT}/{self.MYSQL_DATABASE}?charset=utf8mb4&local_infile=1")
        conn = engine.connect()
